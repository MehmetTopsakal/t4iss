...
class mXANES:
    ...
    # data can be two-column plain text file or [x,y] array
    
    def __init__(self, data=None, usecols=(0,1), structure=None, srange=None,
                 ca=None, edge=None, xanesid=None, source=None, 
                 Eonset=None, 
                 multiplicity=1, calc_vcn=True,
                 lgf=None,verbose=True):
        
        super(mXANES, self).__init__()
                
        if isinstance(data, str):
            try:
                data = np.loadtxt(data, unpack=True, comments='#', usecols=usecols)
                self.E0 = np.array(data[0])
                self.I0 = np.array(data[1])       
                if srange:
                    sel = (self.E0 >= srange[0]) & (self.E0 <= srange[1])
            ...
            try:
                ...
                if verbose:
                    print("empty E,I data")
                self.E0 = []               
                self.I0 = []  
 
        if structure:
            if 'poscar' in structure:
                try:
                    self.structure = mg.Structure.from_file(structure['poscar'])
                    
                    if 'isite' in structure:
                        self.isite = structure['isite']
                    else:
                        self.isite = [] 
                    ...
                    if lgf:
                        try:
                            ...
                            se = lgf.compute_structure_environments(maximum_distance_factor=1.2, only_cations=False,only_indices=[self.isite])
                            lse = LightStructureEnvironments.from_structure_environments(strategy=strategy,structure_environments=se)
                            coor = lse.coordination_environments
                            self.cesym = [coor[self.isite][0]['ce_symbol'], coor[self.isite][0]['csm']]
                            
                            if calc_vcn:
                                try:
                                    from pymatgen.analysis.local_env import VoronoiNN
                                    nnfinder = VoronoiNN(cutoff=6,allow_pathological=True)
                                    vcn = nnfinder.get_cn(self.structure, self.isite, use_weights=True)
                                    self.vcn = vcn
                                except Exception as exc:
                                    print(exc)
                                    if verbose:
                                        print("unable to calculate vcn")
                                    self.vcn = []
                        except Exception as exc:
                            print(exc)
                            if verbose:
                                print("unable to find cesynm")
                            self.cesym = [] 
                            self.vcn = []
                    else:
                        self.cesym = []
                    ...
                    print(exc)
                    if verbose:
                        print("unable to read strucutural information") 
                    self.structure = []
                    self.cesym = []
                    self.vcn = []
        else:
