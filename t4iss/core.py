...
class mXANES:
    ...
    def __init__(self, data=None, data_loadfrom=None, srange=None,
        ...
        if data is None:
            if data_loadfrom:
                data = np.loadtxt(data_loadfrom, unpack=True, 
                                  comments='#', skiprows=0)
        ...
        if structure:
            try:               
                ...
                nnfinder = VoronoiNN(cutoff=vcncutoff,
    ...
    def broaden0(self, g_sigma=None, g_fwhm=None, l_gamma=None, l_fwhm=None,
        ...
        if lvl:            
            if len(lvl) == 1:
                ...
                self.E0, self.I0 = mconv(self.E0, self.I0).\
                                   LorentzianVL(A=lvl[0],B=lvl[1],offset=None)               
            else:
                self.E0, self.I0 = mconv(self.E0, self.I0).\
    ...
    def get_nn(self, nnradius=10.01, axin=None, yshift=0, ms=9, lp='k-',ts=0,
        ...
        if self.structure:
            ...
            if not atoms_off:
                ...
                at = AnchoredText('\n'.join(atext[::-1]),
                           loc=2, prop=dict(size=s), frameon=False,
                           bbox_to_anchor=(0., 1.),
    ...
    if os.path.isdir(path):
        ...
        for i,s in enumerate(sites):
            if s[0].species_string is absorption_specie:   
                ...
                fe = 'exciting_{:03d}_{}-K'.\
                     format(indices[i][0]+1,absorption_specie) 
                fo = 'ocean_{:03d}_{}-K'.\
                     format(indices[i][0]+1,absorption_specie)                     
                ff = 'feff_{:03d}_{}-K'.\
                     format(indices[i][0]+1,absorption_specie)  
